...
/// minutes. Nothing checking the transaction reads a clock, so time is set as a
/// slot *window* on the transaction instead. Here we DO know the wall clock, so
/// we convert "now + 2 min" into a slot number and cap the transaction with an
/// upper bound (`invalidHereafter`, also called the TTL). Submit within the
/// window and it works; miss it and the ledger rejects it, no clock required.
