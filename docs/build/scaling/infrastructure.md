...
Run your own node when you can't afford to trust a third party (high-security apps), when you operate an indexer, or when you're a stake pool operator. Speed up the initial sync with a certified [Mithril](/docs/build/scaling/mithril) snapshot. See [Run your own node](/docs/build/scaling/run-your-own-node) for the developer quick-path (install, run, query) and the handoff to the operator curriculum.
