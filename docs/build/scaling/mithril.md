...
If you operate a stake pool, running a Mithril signer contributes to the certification network. The signer is lightweight (under ~5% CPU, under ~200 MB RAM) and, in production, must route all traffic through a Mithril relay rather than connecting directly to the internet. See [Mithril signer configuration](/docs/operate-a-stake-pool/block-producer/mithril-signer-configuration).
