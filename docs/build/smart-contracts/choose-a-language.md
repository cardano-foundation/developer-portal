...
UPLC is a minimalist, lambda-calculus-based language: variables, functions, function application, constants, a fixed set of built-ins, and an `error` term. No loops, no mutable variables, no objects. That extreme simplicity is intentional: it makes on-chain execution deterministic and the node's evaluator small and auditable. The trade-off is that nobody writes UPLC by hand; you write something higher-level and let a compiler emit it. (If you want the low-level detail, see the [UPLC reference](/docs/build/smart-contracts/advanced/uplc).)
...
[Aiken](/docs/build/smart-contracts/languages/aiken/overview) is a language purpose-built for Cardano validators, with syntax borrowed from Rust, Elm, and Gleam. It has become the most popular choice for new development, and it's where we point newcomers.
...
- **Built-in testing**: a test runner ships with the toolchain, so you write and run unit tests without extra tooling. (See [Testing](/docs/build/smart-contracts/testing).)
...
|---|---|---|
| **[Aiken](/docs/build/smart-contracts/languages/aiken/overview)** | Most new projects | Purpose-built, fast, small output, built-in tests. The default recommendation. |
| **[Plinth](/docs/build/smart-contracts/languages/plinth)** (Plutus Tx) | Haskell teams | The "canonical" language; full Haskell power, on- and off-chain code sharing, mature tooling. Steeper learning curve and larger scripts. |
| **[Plutarch](/docs/build/smart-contracts/languages/plutarch/overview)** | Maximum performance | Fine-grained control close to writing UPLC by hand; almost always the highest performance. Not for the faint-hearted. |
| **[OpShin](/docs/build/smart-contracts/languages/opshin)** | Python teams | Write validators in a subset of valid Python; pairs with PyCardano. |
| **[Scalus](/docs/build/smart-contracts/languages/scalus)** | JVM / Scala teams | Scala 3 for both on-chain and off-chain; works with the JVM and JavaScript. |
| **[Pebble](/docs/build/smart-contracts/languages/pebble)** | TypeScript-familiar teams | Strongly-typed, TypeScript-like syntax that compiles to UPLC. |
| **[Marlowe](/docs/build/smart-contracts/languages/marlowe)** | Financial contracts | A domain-specific language, intentionally **not** Turing-complete, guaranteeing termination; has a visual playground. |
...
This is the concrete reason "smaller, faster scripts" matters, and why Aiken's efficient output is a practical advantage. For tuning, see [Optimization](/docs/build/smart-contracts/advanced/optimization).
...
Blueprints are what let your off-chain code interact with a contract without reading its source: tools can generate TypeScript, Python, or Rust types directly from the blueprint, and different off-chain frameworks can all consume the same format. Aiken generates a blueprint automatically as part of its build. To turn one into type-safe off-chain code, see [Write a validator › from validator to blueprint](/docs/build/smart-contracts/write-a-validator#from-validator-to-blueprint).

## Next steps

- [Lock and spend](/docs/build/smart-contracts/lock-and-spend): write the off-chain transactions that interact with your validator
- [Testing](/docs/build/smart-contracts/testing): test Aiken validators with mock transactions
- [Example contracts](/docs/build/smart-contracts/example-contracts): real validators to read and learn from
