...
The good news: Cardano's [eUTXO model](/docs/value/eutxo) eliminates several of the worst attack classes by design. The rest you handle with careful validator logic and established patterns. This page covers what the platform protects you from, what it doesn't, and how to write validators that hold up.

> Network-level threats (51%, long-range, eclipse attacks) target consensus, not your contract. Cardano's Ouroboros protocol defends against those. See [Consensus & Ouroboros](/docs/foundations/consensus-and-ouroboros). The rest of this page is about application-level security.
...
On Cardano, transaction outcomes are [fully deterministic](/docs/build/smart-contracts/overview#deterministic-validation). A transaction names its exact inputs and outputs; if those inputs still exist when it reaches the chain, it executes exactly as built, otherwise it simply fails: no partial execution, no surprise. This removes entire categories of front-running and MEV attacks.

### Native assets share the ledger's security

On Ethereum, tokens are smart contracts (ERC-20), and every token contract is its own attack surface. On Cardano, [native assets](/docs/native-tokens/overview) are handled by the ledger itself and share ADA's guarantees. The minting policy controls creation, but once tokens exist there is no token contract to exploit.

## Vulnerabilities you still have to guard against

The platform removes some attacks; the rest are your responsibility. These are the big ones, each has a deep-dive in the [vulnerability reference](/docs/build/smart-contracts/advanced/security/overview).
...
**Prevention**: explicitly check that the output datum meets all expected constraints: immutable fields (like ownership) unchanged, mutable fields (like balances) changed only per the allowed rules, and the datum structure matching the expected schema. See [arbitrary datum](/docs/build/smart-contracts/advanced/security/vulnerabilities/arbitrary-datum).
...
**Prevention**: tag outputs with a unique identifier (a state/beacon token) and validate that *your specific* output exists, rather than that "some output" meets the condition. See [double satisfaction](/docs/build/smart-contracts/advanced/security/vulnerabilities/double-satisfaction).
...
See [token security](/docs/build/smart-contracts/advanced/security/vulnerabilities/token-security) and [other token name](/docs/build/smart-contracts/advanced/security/vulnerabilities/other-token-name).

### Resource exhaustion

Validators have [ExUnits budgets](/docs/build/smart-contracts/choose-a-language#what-you-pay-for-execution-costs). An attacker can craft transactions that approach the limits, creating denial-of-service conditions for a protocol. Be conscious of worst-case execution cost; use parameterized scripts, bound loop iterations, and pre-compute expensive work off-chain. See [unbounded inputs](/docs/build/smart-contracts/advanced/security/vulnerabilities/unbounded-inputs) and related entries.
...
- **Deadline enforcement**: use the transaction's [validity range](/docs/build/smart-contracts/advanced/security/vulnerabilities/time-handling) for time-based conditions; it's checked at the protocol level, giving reliable time bounds.
...
The best way to internalize these is to attack them. The [Smart Contract CTF](/docs/build/smart-contracts/advanced/security/ctf) is an interactive Capture-the-Flag where you exploit deliberately vulnerable validators: the fastest way to develop an attacker's eye for your own code.

## Verification: testing, PBT, and audits

Defense in depth, from cheapest to strongest:

- **Unit tests** find the bugs you thought of. See [Testing](/docs/build/smart-contracts/testing).
...
- [Vulnerability reference](/docs/build/smart-contracts/advanced/security/overview): the full catalog with deep-dives
- [Smart Contract CTF](/docs/build/smart-contracts/advanced/security/ctf): practice exploiting and fixing vulnerable validators
- [Testing](/docs/build/smart-contracts/testing): build the test suite that catches these before deployment
