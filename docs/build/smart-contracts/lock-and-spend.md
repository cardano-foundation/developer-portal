...
For real contracts, define the datum with `TSchema` for type safety instead of a raw `Data.constr`. See [Datum, redeemer & context](/docs/build/smart-contracts/datum-redeemer-context).
...
Evolution handles script evaluation, redeemer indexing, and collateral automatically. For time-locked validators, add `.setValidity({ from, to })` so the script can check the current time. See [redeemer indexing](/docs/first-steps/transaction-building#redeemer-indexing) for the static, self, and batch redeemer modes.
...
- The on-chain side (the validator logic and tests) is walked through in [Datum, redeemer & context](/docs/build/smart-contracts/datum-redeemer-context#putting-it-together-a-vesting-example).
