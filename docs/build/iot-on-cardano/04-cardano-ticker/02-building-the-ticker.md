...
- WiFi connectivity from [Workshop 02 - Fetch Wallet Balance](/docs/build/iot-on-cardano/read-and-output/01-fetch-wallet-balance).
- Display techniques from [Workshop 02 - Display Data](/docs/build/iot-on-cardano/read-and-output/02-display-data).
- API fetching from [Workshop 02 - Fetch Wallet Balance](/docs/build/iot-on-cardano/read-and-output/01-fetch-wallet-balance) and [Workshop 03 - Connect and Read Sensor Data](/docs/build/iot-on-cardano/input-and-write/01-connect-and-read-sensor-data).
...
- **`stakeAddress`** - your stake address (`stake1...`). Used for Koios wallet-balance lookups (you learned about stake addresses in [Workshop 02](/docs/build/iot-on-cardano/read-and-output/01-fetch-wallet-balance)).
...
Both modules use the same techniques as [Workshop 02](/docs/build/iot-on-cardano/read-and-output/01-fetch-wallet-balance), just packaged. The data fetcher rate-limits (wallet every 1 minute; tokens / NFTs every 10 minutes) and exposes getters like `getWalletBalance()` and `getToken(i)` that screen files use.
...
The wallet screen shows your ADA balance prominently - same idea as the wallet display from [Workshop 02](/docs/build/iot-on-cardano/read-and-output/02-display-data).
