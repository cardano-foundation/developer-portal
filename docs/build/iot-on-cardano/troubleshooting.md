...
See also: the WiFi sketch in [Workshop 01: Arduino Setup](/docs/build/iot-on-cardano/the-basics/02-arduino-setup) which already includes this workaround.
...
If you don't know the I2C address of your device, run the **I2C scanner sketch** included in [Workshop 03: Connect and Read Sensor Data](/docs/build/iot-on-cardano/input-and-write/01-connect-and-read-sensor-data) - it enumerates every device responding on the bus.
...
2. **ESP32 board package installed** (`Tools → Board → Boards Manager` → search "esp32" → install). See [Arduino Setup](/docs/build/iot-on-cardano/the-basics/02-arduino-setup) for the full setup.
