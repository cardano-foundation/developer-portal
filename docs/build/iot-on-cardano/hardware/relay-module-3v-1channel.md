...
A low-voltage **3V single-channel relay module** with opto-isolation. Switches AC or DC loads safely from a 3.3V microcontroller (ESP32) without needing a level shifter. This is the part used in [Workshop 02 - Light up the Tree](/docs/build/iot-on-cardano/read-and-output/03-light-up-the-tree).
...
A relay can switch mains-voltage loads (110V / 220V), which can cause serious injury or death if mishandled. Read the safety section in [Workshop 02 - Light up the Tree](/docs/build/iot-on-cardano/read-and-output/03-light-up-the-tree) before wiring anything to mains. If unsure, stick to low-voltage LED loads.
:::
