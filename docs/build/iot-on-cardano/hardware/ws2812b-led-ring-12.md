...
A circular **WS2812B LED ring with 12 individually addressable RGB LEDs** (NeoPixel-compatible). Each LED gives 24-bit colour control and the ring is daisy-chainable. Used in [Workshop 02 - Epoch Clock](/docs/build/iot-on-cardano/read-and-output/04-epoch-clock).
...
At full white, each LED can draw up to **60 mA**. A 12-LED ring at full white can draw **720 mA** - more than most USB ports can supply. Run at low brightness when on USB power, or use an external 5V supply rated for ≥ 1 A. Full safety guidance is in [Workshop 02 - Epoch Clock](/docs/build/iot-on-cardano/read-and-output/04-epoch-clock).
:::
