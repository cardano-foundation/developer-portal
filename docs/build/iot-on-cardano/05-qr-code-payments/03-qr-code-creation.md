...
Two libraries needed: **QRcodeDisplay** (the core QR generator) and **QRcode_eSPI** (the TFT_eSPI adapter). Install both - `QRcode_eSPI` depends on `QRcodeDisplay`. You should already have TFT_eSPI installed and configured from [Workshop 02](/docs/build/iot-on-cardano/read-and-output/02-display-data).
