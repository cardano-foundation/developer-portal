...
The Evolution snippets below assume a client set up once from a provider and a wallet (same as for [staking](/docs/build/staking-governance/staking#before-you-start)):
...
To register stake and delegate the vote in one step, use `registerAndDelegateTo({ stakeCredential, drep })`; to do stake + vote together, see [Staking](/docs/build/staking-governance/staking#delegate-stake-and-vote-together).
...
For a browser dApp, [CIP-95](https://cips.cardano.org/cip/CIP-0095) extends the [wallet connector](/docs/build/integrate/connect-a-wallet) with governance methods, so you can read what you need to build governance transactions:
...
See [Connect a wallet](/docs/build/integrate/connect-a-wallet) for the CIP-95 methods on the wallet API.

## Governance in your validators

Plutus V3 added governance **script purposes**: a validator can run as a `Voting` or `Proposing` script, letting a contract participate in governance under script control. See the `ScriptPurpose` list in [Datum, redeemer & context](/docs/build/smart-contracts/datum-redeemer-context#the-scriptpurpose). All the SDK governance operations above also support script-controlled credentials — pass a `redeemer` and `attachScript({ script })`, exactly as for [script-controlled stake](/docs/build/staking-governance/staking#script-controlled-stake-and-the-coordinator-pattern).
