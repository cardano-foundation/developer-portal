...
There are multiple possible setups, in this example we show how [Simple scripts](/docs/build/smart-contracts/write-a-validator#native-scripts-multisig-and-time-locks-without-plutus) can be used for both the *cold* and the *hot* credentials.
...
Create the multisignature `cold.script` file using the simple scrypt syntax. In this example we use the `atLeast` type, so that 2 out of the 3 keys are required for the script to evaluate to true. To learn more about simple scripts read [this article](/docs/build/smart-contracts/write-a-validator#native-scripts-multisig-and-time-locks-without-plutus)
