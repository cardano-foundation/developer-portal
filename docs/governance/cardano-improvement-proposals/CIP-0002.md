--- 
sidebar_label: (2) Coin Selection Algorithms for Cardano
...
The following laws hold:
  ...
  * **_U_**initial   ⊃ **_U_**remaining
  * **_U_**initial   ⊇ **_U_**selected 

And:

  * **_U_**remaining ∩ **_U_**selected  = ∅
  * **_U_**remaining ⋃ **_U_**selected  =
    **_U_**initial  
...
> A given UTxO entry **_u_1** with
> value **_v_1** can be selected if and only if there is no other
> unselected entry **_u_2** with value **_v_2** where
> **_v_2** > **_v_1**.
...
The algorithm proceeds according to the following sequence of steps:
  ...
  * **Step 2**
    ...
    Compare the total size **_n_**selected of the [selected UTxO
    set](#selected-utxo-set) with the [maximum input
    count](#maximum-input-count) **_n_**max.

      * If **_n_**selected > **_n_**max then:
  ...
  * **Step 3**
    ...
    Compare the total value **_v_**selected of the [selected UTxO
    set](#selected-utxo-set) to the total value **_v_**requested of
    the [requested output set](#requested-output-set):

      * If **_v_**selected requested then go to
        step 1.
      * If **_v_**selected ≥ **_v_**requested then go to
        step 4.
  ...
  * **Step 4**
    ...
    Return a [coin selection](#coin-selection) result where:
      ...
      * If **_v_**selected > **_v_**requested then:
...
**_v_**output and accompanying UTxO selection of value
**_v_**selection, the algorithm generates a _single_ change output
of value **_v_**change, where:
...
For each output of value **_v_**, the algorithm:
 ...
 2.  **Attempts to improve upon the existing UTxO selection** for that output,
     ...
     A selection with value **_v_1** is considered to be an
     _improvement_ over a selection with value **_v_0** if **all**
...
## CIP Information  
This [Informational](CIP-0001#cip-format-and-structure) CIP-0002 created on **2020-05-04** has the status: [Active](CIP-0001#cip-workflow).  
This page was generated automatically from: [cardano-foundation/CIPs](https://github.com/cardano-foundation/CIPs/tree/master/CIP-0002/README.md).