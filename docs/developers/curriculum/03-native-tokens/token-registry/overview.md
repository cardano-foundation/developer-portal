...
Reach for **CIP-26** for static metadata on a fungible token, such as a stablecoin's ticker and decimals. It is free, simple, and widely supported. Reach for **CIP-68** when metadata must change over time or a contract needs to read it on-chain, such as an evolving NFT or a token whose properties a dApp updates. The two are not exclusive: the [Token Metadata Server](/docs/developers/curriculum/native-tokens/token-registry/metadata-server) serves both and falls back from one to the other per field.

For the wider metadata picture, including CIP-25 (NFT metadata in the minting transaction) and CIP-27 (royalties), see [Token metadata & registry](/docs/developers/curriculum/native-tokens/metadata-registry).

## Next steps

- [Register an entry](/docs/developers/curriculum/native-tokens/token-registry/register-an-entry): prepare, submit, update, and remove a registry entry
- [Token Metadata Server](/docs/developers/curriculum/native-tokens/token-registry/metadata-server): query metadata over the REST API
