...
Beyond reading, the parsed body can be rebuilt with `MeshTxBuilder`, or turned into a unit tester via `txParser.toTester()` (see [Mocks and assertions](/docs/developers/curriculum/start-building/local-testing#mocks-and-assertions)).
