...
Under the hood a batcher is a continuously running off-chain service, not a one-shot script, and the contention it spares users it absorbs itself. It keeps its own view of the pending orders; because the chain moves underneath it, before assembling a batch it re-checks that each queued order UTxO still exists, drops any a user has since cancelled or spent, and persists enough state to resume after a crash. When one of its own transactions loses a race for the shared pool UTxO, it [rebuilds from fresh chain state and retries](/docs/developers/curriculum/start-building/transaction-building#resilient-submission-retry-safe). Which orders it includes, and in what sequence, is a matching policy (first-come-first-served, largest-first, or whatever maximises fees), and that choice is where a batcher's fairness lives.

Orders are a vocabulary, not just swaps. Production DEXes process deposits and withdrawals through the same order queue, and some support richer types: **donations** (assets paid into the pool with nothing asked back, distributing value pro-rata to all LPs, an on-chain incentive primitive other protocols can drive automatically); **signed-intent orders**, where the order is posted with its details late-bound and an executor later supplies the concrete terms signed by the order's owner, with the signature covering the whole serialized intent and the intent bound to that specific order UTXO and a validity window, the same bind-to-context rule that stops replay everywhere else; and **price-record orders** that mint a snapshot of the pool price other protocols can read without touching the pool. Snapshots record the price at the *end* of a batch, never mid-batch: anyone sandwiching the snapshot hands an arbitrage opportunity to the next batch.
...
Draw the line at collateral, though, not at atomicity. **Refinancing**, repaying an existing loan with liquidity drawn from a new collateralized loan in the same transaction, works fine: both loans are ordinary UTXO flows, and the transaction balances at construction time like any other. What eUTXO rules out is borrowing with nothing at stake on a promise of same-transaction repayment, not composing debt operations atomically.
