...
In the SDKs you give a wall-clock window and the builder converts it to the on-chain slot range. That conversion is per-network: each one has its own slot length (one second on mainnet, preprod, and preview, configurable on a [local devnet](/docs/developers/curriculum/start-building/development-networks)) and its own genesis `zeroTime`/`zeroSlot`, so the builder has to know which chain you are on.
