...
You already **ran** a native script in [Lecture 3](/docs/developers/onboarding/lectures/beginner/tokens-fungible-and-nfts), the `withOneSignature` minting policy was exactly this: a one-line `sig` rule.
