...
- **It fixes the rules forever.** The simplest policy says "only the holder of this key may mint." An NFT usually uses a policy that can **only ever mint once** (often locked after a deadline), and that's exactly what makes it provably one-of-a-kind.

So when the explorer shows a token, the policy ID tells you _which_ asset it truly is, and hints at what its creator is allowed to do next.
...
Let's mint one. This creates **100 GOLD** under the simplest possible policy, a native script that only your wallet's key can use, and the new tokens land back in your wallet. No smart contract and no collateral, just the connected wallet. Notice how the **policy ID** falls out of hashing that policy:
