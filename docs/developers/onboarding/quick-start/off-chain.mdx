...
"Off-chain" means the code that runs **on your computer** (not on the blockchain). It's the app that assembles each transaction, creating tokens, making an offer, accepting one, and hands it to the wallet to sign and send. It's written in TypeScript with a Cardano SDK (pick one in the tabs below).
...
:::tip What just happened
In one transaction, Bob received his 1 SILVER and Alice walked away with the GOLD. Because both moves were in the same transaction, there was never a moment where one person had both, the trade was all-or-nothing.
:::
...
An [Evolution](https://no-witness-labs.github.io/evolution-sdk/) version of the same steps is coming soon. The contract is identical — only the library calls differ.
...
Next: [put a web page on it](/docs/developers/onboarding/quick-start/frontend).
