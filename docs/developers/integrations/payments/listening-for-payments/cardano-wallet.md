...
description: How to listen for ada payments using the cardano-wallet REST API.
...
This guide assumes that you have basic understanding of `cardano-wallet`, how to use it and that you have installed it into your system. Otherwise we recommend reading [Installing cardano-node](/docs/developers/curriculum/production/node/installing-cardano-node), [Running cardano-node](/docs/developers/curriculum/production/node/running-cardano) guides first, along with the [cardano-wallet repository](https://github.com/cardano-foundation/cardano-wallet) for installing and using it.

This guide also assumes that you have `cardano-node` and `cardano-wallet` running in the background and connected to a [testnet network](/docs/developers/curriculum/start-building/networks-and-test-ada).
...
We will get a **wallet address** to show to the customers and for them to send payments to the wallet. In this case we can use the address to request some `tAda` from the [Cardano Testnet Faucet](/docs/developers/curriculum/start-building/networks-and-test-ada) and simulate a payment:
...
Now simply send at least `1,000,000 lovelace` to this **wallet address** or request some `test ada` funds from the [Cardano Testnet Faucet](/docs/developers/curriculum/start-building/networks-and-test-ada). Once complete, we can now run the code again and we should see a successful result this time.
