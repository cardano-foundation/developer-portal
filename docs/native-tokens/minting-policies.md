...
A minting policy is the rule set that controls who can mint or burn a token, when, and how many. The policy is a script; its hash becomes the token's [Policy ID](/docs/native-tokens/overview#how-tokens-are-identified), so the rules are permanently bound to the token's identity.
...
- [Mint a fungible token](/docs/native-tokens/mint-fungible): a native signature policy in practice
- [Mint an NFT](/docs/native-tokens/mint-nft): a time-locked policy plus CIP-25 metadata
