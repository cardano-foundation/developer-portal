...
A fungible token is a native token minted with a quantity greater than one, where every unit is interchangeable. You define a [minting policy](/docs/native-tokens/minting-policies), mint the supply, and the tokens then move through ordinary transactions. Pick your tool below.
...
- Min-ADA travels with tokens, keep a little ADA in the output ([why](/docs/native-tokens/overview#the-minimum-ada-requirement))
...
Full key, address, and node setup is in [Your first transaction](/docs/first-steps/your-first-transaction). The token-specific steps:
...
- [Mint an NFT](/docs/native-tokens/mint-nft): quantity 1, plus CIP-25 metadata and a time-lock
- [Token metadata & registry](/docs/native-tokens/metadata-registry): give your token a name, ticker, and decimals
