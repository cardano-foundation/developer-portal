...
Keys and wallets are the identity and access layer of Cardano. A seed phrase generates a tree of key pairs, public keys are hashed into the [address](/docs/value/addresses) credentials that lock UTXOs, and wallet software manages it all so users can send, receive, and stake. Whoever holds the private key controls the funds, there is no password reset.
...
Wallets can also sign arbitrary messages (CIP-8 / COSE) to prove address ownership without submitting a transaction, the basis for wallet login. For implementations, see [Wallet authentication](/docs/build/integrate/wallet-authentication/overview).
...
Never bundle a mnemonic or private key into frontend code, and never commit one. On a backend, load keys from a secret manager (AWS Secrets Manager, Azure Key Vault, HashiCorp Vault), use a **read-only** wallet wherever you only need to build transactions, and keep separate keys per environment. Production key handling is covered in [going to production](/docs/build/scaling/going-to-production).
:::
...
- [Addresses](/docs/value/addresses): how these keys become the credentials that lock funds
- [Transactions](/docs/value/transactions): how wallets build, sign, and submit
