...
A running node answers the same queries directly: `cardano-cli query utxo --address <addr>`, `cardano-cli query protocol-parameters`, `cardano-cli query tip`, and `cardano-cli query stake-address-info`. See the [cardano-cli reference](/docs/first-steps/transaction-building#building-with-cardano-cli).
