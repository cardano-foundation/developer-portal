...
Seven modules take you from understanding Cardano to shipping and running a real application. Work through them in order, or jump to whatever you need.

1. **[Cardano Fundamentals](/docs/foundations/overview)** — what a blockchain is, cryptography, Ouroboros consensus, the platform's architecture, and the eUTXO ledger: addresses, wallets & keys, transactions, and fees.
2. **[Start Building](/docs/first-steps/overview)** — pick your tools, get test ADA, build/sign/submit your first transaction, and query the chain.
3. **[Native Tokens & NFTs](/docs/native-tokens/overview)** — mint fungible tokens and NFTs (native, no smart contract required) and attach transaction metadata.
...
- **An active community** of millions of wallets, keen to try new products.
