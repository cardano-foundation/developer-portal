...
You must connect your `cardano-node` to a [testnet network](/docs/first-steps/networks-and-test-ada) and make sure it is fully synchronized. If you are not sure how to do that, It is recommended to read [Running cardano-node](/docs/get-started/infrastructure/node/running-cardano) guide before proceeding.
