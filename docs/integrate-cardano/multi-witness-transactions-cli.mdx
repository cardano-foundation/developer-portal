...
This guide assumes that you have completed the [Exploring Cardano Wallets](../integrate-cardano/creating-wallet-faucet.md) guide. You will need one UTxO sitting at each of the wallets (`payment1.addr` and `payment2.addr`) to complete this guide.

This guide also assumes that you have `cardano-node` running in the background and connected to a [testnet network](../get-started/testnets-and-devnets.md).

:::

### Recap

Let's recap what we did so far. Our goal in the [previous guide](../integrate-cardano/creating-wallet-faucet.md) was to draw `1000 tADA` from the Testnet Faucet and send `250 tAda` from **payment1** to **payment2**.
...
As mentioned above, this guide assumes you completed the [Exploring Cardano Wallets](../integrate-cardano/creating-wallet-faucet.md) guide.<br />
