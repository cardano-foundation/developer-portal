...
This guide assumes that you have basic understanding of `cardano-wallet`, how to use it and that you have installed it into your system. Otherwise we recommend reading [Installing cardano-node](../get-started/cardano-node/installing-cardano-node.md), [Running cardano-node](../get-started/cardano-node/running-cardano.md) and [Exploring Cardano Wallets](../integrate-cardano/creating-wallet-faucet.md) guides first.

This guide also assumes that you have `cardano-node` and `cardano-wallet` running in the background and connected to a [testnet network](../get-started/testnets-and-devnets.md).
